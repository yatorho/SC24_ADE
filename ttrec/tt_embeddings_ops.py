...
class TTLookupFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        *tt_cores: Tuple[torch.Tensor],
        ...
        # batch_count = math.ceil(ctx.nnz_tt / 10.)
        # batch_count = 20000
        # batch_count = ctx.nnz_tt
    ...
    def backward(ctx, d_output: torch.Tensor) -> Tuple[Optional[torch.Tensor]]:
        (
            L,
            indices,
            rowidx,
        ...
        if ctx.sparse:
            if ctx.optimizer in [OptimType.SGD, OptimType.EXACT_SGD]:
                # pyre-fixme[16]
                tt_embeddings.tt_sgd_backward( # type: ignore
                    batch_count,
                    ctx.D,
                    ctx.learning_rate,
                    ctx.tt_p_shapes,
...
class TableBatchedTTEmbeddingBag(torch.nn.Module):
    ...
    def __init__(
        ...
        assert np.prod(np.array(self.tt_q_shapes)) == embedding_dim, (
            f"prod(tt_q_shapes) = {np.prod(np.array(self.tt_q_shapes))} "
        ...
        if use_cache:
            ...
            # print("cache_weight: ", (cache_size, self.embedding_dim))
    ...
    def forward(
        self, indices: torch.Tensor, offsets: torch.Tensor, warmup: bool = True, **kwargs
    ) -> torch.Tensor:
        ...
        # print("Cache hit rate: ", num_cached / indices.numel())
        self.last_cache_hit_rate = num_cached / indices.numel()
        # pyre-fixme[16]

        batch_count = kwargs.get("batch_count", 20000)
...
class TTEmbeddingBag(TableBatchedTTEmbeddingBag):
    ...
    def forward(
        ...
        return super().forward(indices, offsets, warmup, **kwargs)[
            0
