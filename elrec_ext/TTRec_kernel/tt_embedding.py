...
TT_embedding_cuda = load(name="TTRec_embedding", sources=[
    "/workspace/SC_artifacts_eval/models/TTRec_kernel/kernel_wrap.cpp", 
    "/workspace/SC_artifacts_eval/models/TTRec_kernel/facebook_kernel.cu", 
    ], verbose=True)
...
class TTLookupFunction(torch.autograd.Function):
    ...
    def forward(
        ctx,
        B: int, # embedding length
        D: int, # feature dimention
        tt_p_shapes: List[int],
        tt_q_shapes: List[int],
        tt_ranks: List[int],
        L: torch.Tensor, # multiplied size
        nnz_tt: int,
        nnz_cached: int,
        indices: torch.Tensor,  # index
        rowidx: torch.Tensor, # offset
        ...
        *tt_cores: Tuple[torch.Tensor],
        ...
        # sorted, _ = torch.sort(indices, 0)
        # pyre-fixme[16]
        output = TT_embedding_cuda.Facebook_TT_forward(
            batch_count,
    ...
    def backward(ctx, d_output: torch.Tensor) -> Tuple[torch.Tensor]:
        ...
        batch_count = 2048
        # print("TT backward",ctx.learning_rate)
        # print(d_output.shape)

        if ctx.sparse:
            if ctx.optimizer in [OptimType.SGD, OptimType.EXACT_SGD]:
                # pyre-fixme[16]
                TT_embedding_cuda.Facebook_TT_sgd_backward(
                    batch_count,
                    ctx.D,
                    ctx.learning_rate,
                    ctx.tt_p_shapes,
            ...
            # pyre-fixme[16]
            d_tt_cores = TT_embedding_cuda.Facebook_TT_sgd_backward_dense(
                batch_count,
                ctx.D,
                ctx.tt_p_shapes,
                ctx.tt_q_shapes,
...
class TableBatchedTTEmbeddingBag(torch.nn.Module):
    ...
    def forward(
        ...
        (
            ...
            cache_locations,
            # pyre-fixme[16]
        ) = TT_embedding_cuda.preprocess_indices_sync(
            indices,
            offsets,
            self.num_tables,
            self.warmup,
...
class TTEmbeddingBag(TableBatchedTTEmbeddingBag):
    ...
    def copy_tt_cores(self, tt_cores):
        for i in range(len(tt_cores)):
            # print(tt_cores[i].unsqueeze(0))
            self.tt_cores[i] = torch.nn.Parameter(tt_cores[i].clone().unsqueeze(0))